...
@Command(scope = "ob", name = "uno-cleanup", description="Destroys the uno script engine and destroys the stores (need init to proceed.)")
public class CleanupCommand extends OsgiCommandSupport {
	
	static Logger logger = LoggerFactory.getLogger(CleanupCommand.class);
    ...
    @Argument(index = 1, name = "resetFlag", description = "Flag (true|false) to reset the mandator", required = false, multiValued = false)
	...
	protected Object doExecute() throws Exception {
        ...
        logger.info("Uno cleanup mandator-id: '{}'", mandatorId);
		...
		if(mandator!=null) {
			...
			baseService.killEngineBase();
			if(resetFlag==null ? false : Boolean.parseBoolean(resetFlag)) {
				baseService.initEngineBase();
			}
		}//fi
